...
app.get('/csv/:userId', async (req, res) => {
  ...
  data.forEach(r => {
    ...
    const type = r.type === 'uri' ? '収入' : '支出';
    rows.push(`"${jst}","${type}","${r.amount}","${r.memo || ''}"`);
  });

  res.setHeader('Content-Type', 'text/csv; charset=utf-8');
  res.setHeader('Content-Disposition', 'attachment; filename=household.csv');
  res.send('\uFEFF' + rows.join('\n'));
});
...
function parse(text) {
  // 収入登録（売上・収入 両方対応）
  const uriMatch = text.match(/^(売上|収入)\s*([0-9０-９,，]+)\s*(.*)/);
  if (uriMatch) {
    const amount = toNumber(uriMatch[2]);
    if (!amount)           return { type: 'error', msg: '⚠️ 金額は正しい数字で入力してください。\n例：収入 15000 給料' };
    if (amount === 'over') return { type: 'error', msg: '⚠️ 1回の登録上限は¥99,999,999です。\n分けて入力してください。' };
    return { type: 'uri', amount, memo: uriMatch[3].trim() };
  }

  // 支出登録（経費・支出 両方対応）
  const expMatch = text.match(/^(経費|支出)\s*([0-9０-９,，]+)\s*(.*)/);
  if (expMatch) {
    const amount = toNumber(expMatch[2]);
    if (!amount)           return { type: 'error', msg: '⚠️ 金額は正しい数字で入力してください。\n例：支出 3200 食費' };
    if (amount === 'over') return { type: 'error', msg: '⚠️ 1回の登録上限は¥99,999,999です。\n分けて入力してください。' };
    return { type: 'exp', amount, memo: expMatch[3].trim() };
  }
...
async function respond(cmd, userId) {
  ...
  if (cmd.type === 'yearly') {
    ...
    for (let m = 1; m <= jstNow.month; m++) {
      ...
      results.push(`${m}月　収支 ${profit >= 0 ? '' : '-'}${yen(Math.abs(profit))}`);
    }
  ...
  if (cmd.type === 'yearlySummary') {
    ...
    for (let m = 1; m <= maxMonth; m++) {
      ...
      totalUri += uri;
      totalExp += exp;
      results.push(`${m}月　収入 ${yen(uri)}　支出 ${yen(exp)}`);
    }
  ...
  if (cmd.type === 'subsidy') {
    ...
    const salesText = estimatedAnnual > 0 ? `\n（年間収入推定：${yen(estimatedAnnual)}をもとにマッチング）` : '';
  ...
  if (cmd.type === 'undo') {
    ...
    const label = last.type === 'uri' ? '収入' : '支出';
...
app.post('/daily-report', async (req, res) => {
  ...
  try {
    const now = new Date();
    const jstNow = toJSTDate(now);
    const todayStart = `${jstNow.year}-${pad(jstNow.month)}-${pad(jstNow.day)}T00:00:00+09:00`;
    const todayEnd   = `${jstNow.year}-${pad(jstNow.month)}-${pad(jstNow.day)}T23:59:59+09:00`;
    const monthStart = `${jstNow.year}-${pad(jstNow.month)}-01T00:00:00+09:00`;

    // 過去に1件でも記録があるユーザー全員を取得
    ...
    for (const userId of userIds) {
      try {
        ...
        const todayIncome = todayData ? todayData.filter(r => r.type === 'uri').reduce((s, r) => s + r.amount, 0) : 0;
        const todayExp    = todayData ? todayData.filter(r => r.type === 'exp').reduce((s, r) => s + r.amount, 0) : 0;
        const todayProfit = todayIncome - todayExp;
        const todayProfitStr = todayProfit >= 0 ? yen(todayProfit) : `-${yen(Math.abs(todayProfit))}`;
        ...
        // 今日記録なしの場合はひと言添える
        const todayNote = (todayIncome === 0 && todayExp === 0)
          ? '今日はまだ記録がありません📝\n「収入 金額」や「支出 金額」で記録してみましょう！\n\n'
          : '';

        const message =
`🌙 ${jstNow.month}/${jstNow.day} 今日のまとめ
─────────────────
${todayNote}収入　　${yen(todayIncome)}
支出　　${yen(todayExp)}
─────────────────
本日収支　${todayProfitStr}
